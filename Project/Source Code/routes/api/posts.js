...
router.get("/", (req, res) => {
  Post.find()
    ...
    .catch(err =>
      res.status(404).json({ nopostsfound: "No posts with that id" })
    );
});
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      Post.findById(req.params.id)
        .then(post => {
          ...
          //delete
          post.remove().then(() => re.json({ success: true }));
        })
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      Post.findById(req.params.id)
        .then(post => {
          ...
          //add like
          const removeIndex = post.likes.map(item => item.user.id);
...
//router.get('/test', (req, res) => res.json({msg: "posts is working fine"}));
